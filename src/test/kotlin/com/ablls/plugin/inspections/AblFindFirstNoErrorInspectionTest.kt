...
class AblFindFirstNoErrorInspectionTest : BasePlatformTestCase() {
    ...
    fun testFindFirstWithoutNoErrorTriggersWarning() {
        val descr =
            "FIND FIRST/LAST without NO-ERROR throws a fatal error " +
                "when no record is found — add NO-ERROR and check AVAILABLE"
        myFixture.configureByText(
            "test.p",
            """<warning descr="$descr">FIND</warning> FIRST Customer WHERE Customer.CustNum = 1 NO-LOCK.""",
        )
    ...
    fun testFindLastWithoutNoErrorTriggersWarning() {
        ...
        myFixture.configureByText(
            "test.p",
            """<warning descr="$descr">FIND</warning> LAST Customer WHERE Customer.CustNum = 1 NO-LOCK.""",
        )
        myFixture.checkHighlighting(true, false, false)
    }
