...
object AblBuiltinDocs {
    private val DOCS: Map<String, String> =
        buildMap {
            ...
            put(
                "ERROR-STATUS",
                "**ERROR-STATUS** — État d'erreur après `NO-ERROR`.\n\n" +
            ...
            put(
                "READ-XML",
                "**READ-XML** `(schema-type, source [, read-mode [, schema-source [, lang]]])`" +
            ...
            put(
                "NO-LOCK",
                "**NO-LOCK** — Accès lecture seule sans verrouillage (le plus performant pour les lectures).",
            )
            ...
            put(
                "ABSTRACT",
                "**ABSTRACT** — Modificateur : la méthode/classe doit être implémentée par les sous-classes.",
            )
    ...
    fun get(name: String): Optional<String> = Optional.ofNullable(DOCS[name.uppercase().trim()])

    fun has(name: String): Boolean = DOCS.containsKey(name.uppercase().trim())
}
