...
class AblParserFacade {
        ...
        parser.errorHandler =
            object : DefaultErrorStrategy() {
                override fun reportNoViableAlternative(
                    ...
                    try {
                        super.reportNoViableAlternative(r, e)
                    } catch (
                        _: NullPointerException,
                    ) {
                        r.notifyErrorListeners(e.offendingToken, "no viable alternative", e)
                    }
                }
                ...
                override fun reportInputMismatch(
                    ...
                    try {
                        super.reportInputMismatch(r, e)
                    } catch (
                        _: NullPointerException,
                    ) {
                        r.notifyErrorListeners(e.offendingToken, "input mismatch", e)
                    }
                }

                override fun reportMissingToken(r: Parser) {
                    try {
                        super.reportMissingToken(r)
                ...
                override fun recoverInline(r: Parser): Token {
                    return try {
                        super.recoverInline(r)
                    } catch (_: NullPointerException) {
                        throw InputMismatchException(r)
                    }
                }
            }
    ...
    fun analyze(
        content: String,
        uri: String,
    ): AblSemanticResult = analyze(parse(content, uri))
