    ...
    override fun buildVisitor(
        ...
        object : PsiElementVisitor() {
            override fun visitFile(file: PsiFile) {
                ...
                while (i < size - 2) {
                    val t1 = tokens.get(i)
                    if (t1.channel != Token.DEFAULT_CHANNEL) {
                        i++
                        continue
                    }
                    ...
                    if (tableName.uppercase() in SKIP_IDENTIFIERS) {
                        i++
                        continue
                    }
                    ...
                    if (!knownTables.contains(tableName.uppercase())) {
                        i++
                        continue
                    }
                    ...
                    val fieldExists =
                        service.symbolIndex.allSymbols().any { sym ->
                    ...
                    if (!fieldExists) {
                        val range =
                            AblInspectionHelper.toRange(
                                doc,
                                fieldToken.line,
                                fieldToken.charPositionInLine,
                                fieldName.length,
                            )
