...
class AblUnusedVariableInspection : LocalInspectionTool() {
    ...
    private fun isParameter(variable: Any): Boolean {
        return variable.javaClass.simpleName.contains("Parameter", ignoreCase = true)
    }
    ...
    private fun checkFallback(
        ...
        while (i < size) {
            ...
            if (j >= size || !tokens.get(j).text.equals("VARIABLE", ignoreCase = true)) {
                i++
                continue
            }
            ...
            if (name == null || name.startsWith("_") || name.uppercase() in ABL_KEYWORDS) {
                i++
                continue
            }
