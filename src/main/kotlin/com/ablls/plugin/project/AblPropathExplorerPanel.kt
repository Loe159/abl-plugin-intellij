...
class AblPropathExplorerPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    fun refresh() {
        ...
        config.propath.forEach { pathStr ->
            ...
            val dirNode = DefaultMutableTreeNode(
                NodeInfo(resolved, if (exists) NodeKind.DIR_OK else NodeKind.DIR_MISSING, path?.toString() ?: resolved)
            )
            root.add(dirNode)
    ...
    private inner class PropathTreeCellRenderer : DefaultTreeCellRenderer() {
        override fun getTreeCellRendererComponent(
            ...
            when (info?.kind) {
                NodeKind.DIR_OK      -> { icon = AllIcons.Nodes.Folder; text = info.label }
                NodeKind.DIR_MISSING -> { icon = AllIcons.Nodes.Folder; foreground = java.awt.Color.RED; text = info.label }
                NodeKind.FILE        -> {
                    icon = when {
