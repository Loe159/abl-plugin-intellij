...
MODULE_PATH = CHECKS_DIR / "prove_implementation_quality_gate_validation.py"
REPO_ROOT = CHECKS_DIR.parents[1]
sys.path.insert(0, str(CHECKS_DIR))
SPEC = importlib.util.spec_from_file_location(
    "prove_implementation_quality_gate_validation",
    MODULE_PATH,
)
...
class ProveImplementationQualityGateValidationTest(unittest.TestCase):
    def test_policy_is_exact_and_keeps_execution_unproven(self) -> None:
        policy = proof.load_policy()

        self.assertEqual("quality_gate_receipt_validation", policy["proven_control"])
        self.assertIn("implementation_quality_gate_execution", policy["unproven_controls"])

    def test_real_synthetic_receipts_match_all_fixtures(self) -> None:
        ...
        self.assertFalse(result["scope"]["proves_historical_output_authenticity"])
