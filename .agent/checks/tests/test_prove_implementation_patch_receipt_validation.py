...
MODULE_PATH = CHECKS_DIR / "prove_implementation_patch_receipt_validation.py"
REPO_ROOT = CHECKS_DIR.parents[1]
sys.path.insert(0, str(CHECKS_DIR))
SPEC = importlib.util.spec_from_file_location(
    "prove_implementation_patch_receipt_validation",
    MODULE_PATH,
)
...
class ProveImplementationPatchReceiptValidationTest(unittest.TestCase):
    def test_policy_is_exact_and_keeps_quality_gate_unproven(self) -> None:
        policy = proof.load_policy()

        self.assertEqual("implementation_patch_receipt_validation", policy["proven_control"])
    ...
    def test_real_synthetic_repositories_match_all_fixtures(self) -> None:
        ...
        self.assertFalse(result["scope"]["proves_historical_producer"])
